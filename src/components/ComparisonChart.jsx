...
export default function ComparisonChart({ history }) {
  ...
  const maxY = Math.max(2, ...history.map((h) => Math.max(h.fixedAvg, h.aiAvg))) * 1.15;
  ...
  return (
    <div className="chart-wrap">
      <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="comparison-chart" preserveAspectRatio="none">
        ...
        <path d={buildPath(history, xScale, yScale, "fixedAvg")} className="chart-line chart-line-fixed" />
        <path d={buildPath(history, xScale, yScale, "aiAvg")} className="chart-line chart-line-ai" />
      </svg>
